...
vector<vector<vector<double> > > attribute_matrix, attribute_tmp, A, A_tmp;
...
void norm_rel(int e1, int rel)
{
	while (true)
	{
		...
		for (int j=0; j<m; j++)
		{
			double tmp = 0;
			for (int i=0; i<n; i++)
				tmp += entity_tmp[e1][i]*A_tmp[rel][i][j];
			syn1.push_back(tmp);
		}
		double y = 0;
		for (int j=0; j<m; j++)
		...
		if (y>1)
		{
			...
			for (int j=0; j<m; j++)
			{
				...
				for (int i=0; i<n; i++)
				{
					...
					entity_tmp[e1][i] -= 1*rate*lambda*x*A_tmp[rel][i][j];
					A_tmp[rel][i][j] -= 1*rate*lambda*x*tmp_val;
				}
			}
		}
		else
...
void gradient(double sig, int e1,int e2,int rel, int y)
{
	for (int ii=0; ii<m; ii++)
    {
        ...
        for (int jj=0; jj<n; jj++)
        {
            A_tmp[rel][jj][ii]+=x*(entity_vec[e1][jj]-entity_vec[e2][jj]);
            entity_tmp[e1][jj]+=x*A[rel][jj][ii];
            entity_tmp[e2][jj]-=x*A[rel][jj][ii];
        }
        relation_tmp[rel][ii]+=x;
    }
}

double calc_sum(int e1,int e2,int rel, int y)
{
	//cout<<269<<' '<<e1<<' '<<e2<<' '<<rel<<' '<<y<<endl;
    double sum=0;
...
//	cout<<220<<endl;
    ...
    for (int epoch=0; epoch<nepoch; epoch++)
    {
		...
		time_begin();
     	for (int batch = 0; batch<nbatches; batch++)
     	{
			//cout<<batch<<' '<<res<<endl;
     		...
     		{
				//cout<<k<<' '<<batchsize<<endl;
				int i=rand_max(fb_h.size());
				int e1 = fb_h[i],rel = fb_r[i], e2  = fb_l[i];
				int j=rand_max(entity_num);
				calc_sum(e1,e2,rel, 1);
				for (int neg_id = 0; neg_id<c1; neg_id++)
				{
					j=rand_max(entity_num);
					double pr = rand()%1000;//1000*right_mean[fb_r[i]]/(right_mean[fb_r[i]]+left_mean[fb_r[i]]);
		...
		cout<<endl;
        cout<<"epoch:"<<epoch<<' '<<res<<' '<<relation_bias<<' '<<attr_beta_bias<<endl;
		time_end();
		res1 = 0;
		...
		fclose(f_attr2);
		FILE* f_A = fopen(("A"+version+".txt").c_str(), "w");
        for (int i=0; i<relation_num; i++)
        for (int jj=0; jj<n; jj++)
        {
            for (int ii=0; ii<m; ii++)
            {
                fprintf(f_A,"%.6lf\t",A[i][jj][ii]);
            }
            fprintf(f_A,"\n");
        }
		fclose(f_A);
...
//	cout<<397<<endl;
	...
	string version1 = "0";
    FILE* f2 = fopen(("../new_method/relation2vec"+version1+".txt").c_str(),"r");
    FILE* f3 = fopen(("../new_method/entity2vec"+version1+".txt").c_str(),"r");
    for (int i=0; i<relation_num; i++)
        for (int ii=0; ii<n; ii++)
    ...
    fclose(f2);
    fclose(f3);
	FILE* f_val = fopen(("../new_method/val2vec"+version1+".txt").c_str(),"r");
	for (int i=0; i<val_num; i++)
		for (int ii=0; ii<m; ii++)
			fscanf(f_val, "%lf", &val_vec[i][ii]);
	FILE* f_attr = fopen(("../new_method/attr2matrix"+version1+".txt").c_str(),"r");
	for (int i=0; i<attribute_num; i++)
		for (int ii=0; ii<n; ii++)
			for (int jj=0; jj<m; jj++)
				fscanf(f_attr, "%lf", &attribute_matrix[i][ii][jj]);
	fclose(f_attr);
	FILE* f_attr1 = fopen(("../new_method/attr2vec"+version1+".txt").c_str(),"r");
	for (int i=0; i<attribute_num; i++)
			for (int jj=0; jj<m; jj++)
				fscanf(f_attr1, "%lf", &attribute_vec[i][jj]);
	fclose(f_attr1);
	FILE* f_attr2 = fopen(("../new_method/attr2beta"+version1+".txt").c_str(),"r");
	for (int i=0; i<attribute_num; i++)
			for (int jj=0; jj<nn; jj++)
